...
import { convertExports, convertImportOrRequire, convertImportToDcRequire, convertRelative, convertRequireCallToImport, dc, dcRequire, ExportArray, LVal, mustIdent, replaceJsxElement, replaceWithIdent, TransformOptions } from "./util";
	...
	const visitor: Visitor = {
		...
		visitVariableDeclaration(path) {
			...
			if (res.length) {
				path.replace(
					...res.map(([mod, specs]) =>
						b.importDeclaration(specs, b.stringLiteral(mod))
					)
				);
			}
		},
		visitCallExpression(path) {
			...
			if (
				dcImports.find(
					(a) =>
						t.Identifier.check(a.imported) &&
						a.imported.name == (node.callee as N.Identifier).name
				) != null ||
				dcHooks.find(
					(a) =>
						t.Identifier.check(a.imported) &&
		...
		visitAssignmentExpression(path) {
			...
			const isModuleExports = (
				node: LVal | N.OptionalMemberExpression
			): boolean => {
				if (t.MemberExpression.check(node)) {
					if (
						t.Identifier.check(node.property) &&
						t.Identifier.check(node.object)
					) {
						return (
			...
			if (isModuleExports(node.left as LVal)) {
				...
				if (
					t.MemberExpression.check(me.object) &&
					t.Identifier.check(me.object.property)
				) {
				...
				if (
					...
					if (t.ObjectProperty.check(path.parent))
						path.replace(b.nullLiteral());
			...
			if (node.declaration) {
				...
				if (node.source && node.specifiers && node.specifiers.length) {
					...
					if (entry) {
						...
						const specs = replaceWithIdent(
							node.specifiers as N.ExportSpecifier[],
							awaiter
						);
						dcExports.push(
							b.exportNamedDeclaration(
								...
								(specs as N.VariableDeclaration).declarations.flatMap(
									(a) => {
										...
										if (t.Identifier.check(a)) {
											...
											fin.push(
												...a.id.properties.map((c: N.ObjectProperty) =>
													b.exportSpecifier(
